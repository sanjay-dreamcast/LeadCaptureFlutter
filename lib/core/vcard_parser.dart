class VcardParser {
  ...
  Map<String, dynamic> parse() {
    ...
    List<String> lines =
        content.replaceAll(";;", "").split(VCARD_TAG_SEPARATOR);
    lines.forEach((field) {
      String key = "";
      dynamic value;
      ...
      value = tagAndValue[1].trim().replaceAll(
          VCARD_TAG_VALUE_IGONE_SEPARATOR[0],
          VCARD_TAG_VALUE_IGONE_SEPARATOR[1]);
    ...
    rawFields.getRange(1, rawFields.length).forEach((rawField) {
      ...
      if (items.length == 2) {
        field = {
          ...
          'value': items.elementAt(1).replaceAll(
              VCARD_TAG_VALUE_IGONE_SEPARATOR[0],
              VCARD_TAG_VALUE_IGONE_SEPARATOR[1])
        };
      }
